...
function validateVariants(block, path, errors) {
  ...
  for (const variant of block.variant) {
    if (!allowedVariantSet.has(variant))
      errors.push(`${path}: unsupported variant '${variant}'`);
  }
}
...
function validateGrid(block, path, errors) {
  ...
  if (
    !Number.isInteger(block.grid.span) ||
    block.grid.span < 1 ||
    block.grid.span > 14
  ) {
  ...
  if (
    block.grid.align !== undefined &&
    !["start", "center", "end"].includes(block.grid.align)
  ) {
...
function validateChildren(block, path, errors) {
  if (leafTypes.has(block.type)) {
    if (block.children !== undefined)
      errors.push(`${path}: '${block.type}' cannot have children`);
    return [];
  }
...
function validateChildType(parent, child, path, errors) {
  ...
  if (
    ...
    ["list", "listItem"].includes(child.type)
  ) {
    errors.push(
      `${path}: contentSection children must be independent row blocks`,
    );
    return;
  }
  ...
  if (
    containerTypes.has(parent.type) &&
    child.type === "text" &&
    child.text?.trim()
  ) {
    errors.push(
      `${path}: meaningful text must be wrapped in a text block parent`,
    );
  }
}

function validateSectionStructure(block, children, path, errors) {
  const contentSectionBlocks = children.filter(
    (child) =>
      child?.type === "group" && child.variant?.includes("contentSection"),
  );
  ...
  if (block.variant?.includes("content")) {
    if (contentSectionBlocks.length !== 1) {
      errors.push(
        `${path}: the content section must contain exactly one contentSection group`,
      );
    }
  }
}
...
function validateAssetReference(block, field, kind, path, errors, assets) {
  ...
  if (!asset) errors.push(`${path}: ${field} must reference a document asset`);
    ...
    errors.push(`${path}: ${field} must reference a ${kind} asset`);
}
...
function validateBlock(block, path, errors, ids, assets) {
  ...
  if (!block.id || typeof block.id !== "string")
    ...
    errors.push(`${path}: duplicate id '${block.id}'`);
  ...
  if (block.type === "text") {
    if (
      ...
      errors.push(`${path}: text must be a string when provided`);
    if (!Array.isArray(block.marks))
      errors.push(`${path}: marks must be an array`);
    else
      for (const mark of block.marks) {
        if (!allowedMarkSet.has(mark))
          errors.push(`${path}: unsupported mark '${mark}'`);
      }
  }
  if (
    block.type === "heading" &&
  ...
  if (block.type === "spacer" && !allowedSpacerSizeSet.has(block.size)) {
    errors.push(
      `${path}: spacer size must be one of ${allowedSpacerSizes.join(", ")}`,
    );
  }
  if (block.type === "link") {
    if (!block.href || typeof block.href !== "string")
      errors.push(`${path}: href is required`);
    else if (/^(?:javascript|data):/i.test(block.href))
      errors.push(`${path}: unsafe href protocol`);
    if (typeof block.newTab !== "boolean")
  ...
  if (["image", "video"].includes(block.type)) {
    ...
    if (!allowedLayoutSet.has(block.layout)) {
      errors.push(
        `${path}: layout must be one of ${allowedLayouts.join(", ")}`,
      );
    }
    if (
  ...
  if (block.type === "codeBlock") {
    ...
    if (!codeLanguageValueSet.has(block.language)) {
      errors.push(
        `${path}: language must be one of ${codeLanguageValues.join(", ")}`,
      );
    }
  }
...
function validatePageData(document, errors, assets) {
  ...
  if (!Array.isArray(document?.relatedProjects))
    ...
    for (const [index, project] of document.relatedProjects.entries()) {
      ...
      if (relatedSlugs.has(project?.slug))
        errors.push(`${path}: duplicate slug '${project.slug}'`);
...
export function validateProjectDocument(document) {
  ...
  for (const key of ["projectId", "slug", "title"]) {
    if (!document[key] || typeof document[key] !== "string")
      errors.push(`${key} is required`);
  }
  if (typeof document.excerpt !== "string")
  ...
  if (
    document.contentWidth !== undefined &&
    !["large", "medium", "small"].includes(document.contentWidth)
  ) {
  ...
  if (!Array.isArray(document.meta)) errors.push("meta must be an array");
    ...
    for (const [index, item] of document.meta.entries()) {
      ...
      if (metaIds.has(item?.id))
        errors.push(`meta[${index}]: duplicate id '${item.id}'`);
      metaIds.add(item?.id);
    }
  }
  ...
  for (const [index, asset] of (document.assets ?? []).entries()) {
    ...
    if (assets.has(asset.id))
      errors.push(`${path}: duplicate id '${asset.id}'`);
    assets.set(asset.id, asset);
    if (!assetKindSet.has(asset.kind))
      errors.push(`${path}: unsupported kind '${asset.kind}'`);
    if (!assetProviderSet.has(asset.provider))
      errors.push(`${path}: unsupported provider '${asset.provider}'`);
  ...
  for (const [index, block] of (document.blocks ?? []).entries()) {
    if (block?.type && !rootTypes.has(block.type)) {
      errors.push(
        `blocks[${index}]: '${block.type}' is not allowed at document root`,
      );
    }
